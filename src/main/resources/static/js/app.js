...
const dashboard = document.getElementById('recipe-dashboard');
const searchVal = document.querySelector('#searchVal');
const subcat = Array.from(document.getElementsByClassName('search-by-subcat'));
const cat = Array.from(document.getElementsByClassName('cat'));
const recipeDash = document.getElementById('recipe-dash');
...
window.addEventListener('load', (event) => {
    ...
    //     const recipes = Array.from(document.getElementsByClassName('food-item lorem-type'));
    ...
    //         const prodName = formData.get('products');
    //         const category = formData.get('category');
    //         const portions = formData.get('portions');
    //
    //         if (!prodName || !category || !portions) {
    //             return alert('Всички полета са задължителни!');
    //         }
    //         fetch("http://localhost/programming/Let'sCook/api/product/read_by_quantity.php?name=" + prodName + "&measure_unit=" + portions + "&quantity=" + category, {
    //             method: "POST",
    //             headers: {
    ...
    fetch("http://localhost/programming/Let'sCook/api/recipe/read_single.php?recipe_id=" + id, {
        ...
        fetch("http://localhost/programming/Let'sCook/api/photo/read.php?recipe_id=" + id, {
            ...
            for (var i = 0; i < jsonData.length; i++) {
                ...
                photoCon += '> <div class="img recipe-hero-img"> </div> <img src="' + photo.image_url + '" style="width:100%"> </div>';
            }
